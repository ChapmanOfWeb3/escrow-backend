...
export class IndexerRunnerFailureMonitor {
  ...
  constructor(options: { name?: string; failureThreshold?: number } = {}) {
    this.runner = options.name ?? "indexer_runner";
  ...
  recordFailure(failureType: string, details: { error?: string } = {}): number {
    this.consecutiveFailures += 1;
